...
def create_context_menu(target, player, city, get_sprite_coordinates,  NonBlockingPopupMenu, sprite=None):
    ...
    if target == 'item':
        ...
        if sprite is not None:
            menu_item = sprite
    elif target == 'current block' and is_building(player, city):
    ...
    elif target == 'current block' and not is_building(player, city):
...
def handle_menu_action(player, update_viewport, menu_name, action, chat_history):
    if menu_name == 'Item':
        ...
        if action == 'Barricade':
            ...
            player.inside = True
            player.button_group.remove(player.enter_button)
            player.button_group.add(player.leave_button)
        elif action == 'Leave':
            player.inside = False
            player.button_group.remove(player.leave_button)
